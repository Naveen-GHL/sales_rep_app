...
public sealed class ExecutiveDashboardService(ApplicationDbContext context, ICurrentUserService currentUser) : IExecutiveDashboardService
{
    public async Task<ExecutiveDashboardDto> GetAsync(CancellationToken cancellationToken)
    {
        ...
        var leads = context.Set<Lead>().AsNoTracking().Where(x => x.CompanyId == currentUser.CompanyId && x.AssignedToUserId == currentUser.UserId);
        var followups = context.Set<Followup>().AsNoTracking().Where(x => x.CompanyId == currentUser.CompanyId && x.UserId == currentUser.UserId);
        ...
        return new ExecutiveDashboardDto {
            ...
            AverageTalkTimeSeconds = await todayCalls.Select(x => (double?)x.DurationSeconds).AverageAsync(cancellationToken) ?? 0,
