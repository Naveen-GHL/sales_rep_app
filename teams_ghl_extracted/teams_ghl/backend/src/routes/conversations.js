...
function getConversationsForUser(userId, companyId) {
  const compId = companyId || 't-ghl-01';
...
router.get('/', authenticate, (req, res) => {
  const conversations = getConversationsForUser(req.user.id, req.user.company_id);
...
router.post('/', authenticate, (req, res) => {
  ...
  if (type === 'dm') {
    ...
    const existing = db.prepare(`
      ...
      WHERE c.type = 'dm' AND (c.company_id = ? OR c.company_id IS NULL)
      LIMIT 1
    `).get(a, b, companyId);
    if (existing) {
      const conv = getConversationsForUser(req.user.id, companyId).find(c => c.id === existing.id);
      return res.status(200).json(conv || existing);
    }
  }

  const id = uuidv4();
  db.prepare(`INSERT INTO conversations (id, type, name, created_by, company_id) VALUES (?, ?, ?, ?, ?)`)
    .run(id, type, name || '', req.user.id, companyId);
